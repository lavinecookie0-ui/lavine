...
export default function BusinessSupportPage() {
  ...
  const faqs = [
    ...
    "Cari limit hatası alıyorum",
    "Dolap derecesi giremiyorum",
    "Çark hakkım görünmüyor",
    "Sipariş durumumu göremiyorum"
  ];

  const handleWhatsApp = () => {
    if (!settings?.whatsappPhone) return;
    const userName = currentUser?.displayName || 'İşletme';
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0, color: '#f1f1f5', letterSpacing: '-0.02em' }}>Destek Merkezi</h1>
        <p style={{ margin: '8px 0 0', color: '#9898a8', fontSize: 14 }}>Lavine B2B panel kullanımıyla ilgili yardıma mı ihtiyacınız var?</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20, marginBottom: 40 }}>
        ...
        {settings.isWhatsAppEnabled && settings.whatsappPhone && (
          ...
          >
            ...
            <p style={{ fontSize: 13, color: '#9898a8', margin: 0 }}>Hızlı yanıt için WhatsApp üzerinden bizimle iletişime geçin.</p>
          </div>
        )}
        ...
        <div style={{ background: '#16161e', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 24, height: 'fit-content' }}>
          ...
          <div style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <p style={{ fontSize: 13, color: '#5c5c70', margin: 0, lineHeight: 1.5 }}>
              Sistemsel sorunlar veya acil sipariş destekleriniz için mesai saatleri içerisinde bize ulaşabilirsiniz.
            </p>
          </div>
        </div>
      </div>
