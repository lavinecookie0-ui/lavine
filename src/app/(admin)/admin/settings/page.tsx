...
import { subscribeToSettings, subscribeToSupportSettings, updateSettings, updateSupportSettings } from '@/lib/firebase/firestore';
import { Settings as SettingsType, SupportSettings } from '@/types';
import toast from 'react-hot-toast';
import { useAuth } from '@/contexts/AuthContext';

export default function AdminSettingsPage() {
  const { userData, currentUser } = useAuth();
  const [activeTab, setActiveTab] = useState<'points' | 'support'>('points');
  ...
  useEffect(() => {
    const unsubPoints = subscribeToSettings((data: SettingsType) => setPointsRate(data.pointsRate.toString()));
    const unsubSupport = subscribeToSupportSettings((data) => setSupportData(data));
    return () => { unsubPoints(); unsubSupport(); };
  }, []);
